module While.Domain.Int.SemanticsSpec (spec) where
...
spec =
    ...
    describe "interpretBExpr" $ do
        ...
        it "interprets (x == 2)" $
            interpretBExpr (parseBExpression "x == 2") (top :: State Domain)
                `shouldBe` storeList [("x", Val 2)] (top :: State Domain)
    ...
    describe "interpretState" $ do
        ...
        context "seq" $
            it "interprets x := 1; x := 2" $
                interpretState (parseString "x := 1; x := 2") (empty :: State Domain)
                    `shouldBe` store "x" (Val 2) (empty :: State Domain)

        context "if" $ do
            it "interprets x := 1; if x == 2 then x := 3 else skip)" $
                interpretState
                    (parseString "x := 1; if x == 2 then x := 3 else skip")
                    (empty :: State Domain)
                    `shouldBe`
                    storeList [
                            ("x", Val 1)
            ...
            it "interprets y := 1; if y == 2 then x := 3/0 else x := 4" $
                interpretState
                    ...
                    (empty :: State Domain)
                    `shouldBe`
                    store "y" (Val 1) (store "x" (Val 4) (empty :: State Domain))
            ...
            it "interprets y := 1; if y == 1 then x := 2/0 else x := 3/0" $
                interpretState
                    ...
                    (empty :: State Domain)
                    `shouldBe`
                    store "y" (Val 1) (store "x" bottom (empty :: State Domain))
        ...
        context "while" $ do
            it "interprets while x != 2 then x := 2" $
                interpretState
                    ...
                    (empty :: State Domain)
                    `shouldBe`
                    store "x" (Val 2) (empty :: State Domain)
            ...
            it "interprets x := 1; while x == 1 do skip" $
                interpretState
                    ...
                    (empty :: State Domain)
                    `shouldBe`
                    store "x" bottom (empty :: State Domain)
        ...
        it "interprets complex program" $
            interpretState
                (parseString "x := 0; if x < 10 then a := 2 else b := 3;")
                (empty :: State Domain)
                `shouldBe`
                storeList [
                        ("x", Val 0),
